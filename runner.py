...
import tornado.tcpserver as tc
...
class easyhttpbenchmark(object):
 ...
 # process.fork_processes(options.processnum) # for liunx
    ...
    def benchmark_test(self):
        ...
        processnum = options.processnum if options.processnum else 0  #如果启动
        tcpserver = tc.TCPServer()
        tcpserver.start(processnum)
        self._io_loop.start()
...
def main():
    try:
        ...
        # import os
        # os.system("python easystatserver.py")
        ...
        #             filemode='a')
        easyhttpbc=easyhttpbenchmark(options.maxclientnum,options.clientnum,options.testtime,options.flag, options.datafile_json)
